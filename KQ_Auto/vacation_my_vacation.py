...
from vacation_functions import commons,relativedelta,data,datetime,time,timedelta,Keys
from vacation_param import xpath,type_vc,Pass,Fail,All,Con,Am,Pm,Hour,Des


def InfoCc(selected_cc , saved_cc):
    commons.Content(data["cc"]["selected"] % selected_cc)
    commons.Content(data["cc"]["saved"]    % saved_cc)
   
def VacationRequest(vacation_request,i):
    vacation_request["vc_name"]      = xpath.LiRequest(i,"na")
...
def ApproverName(total_app,list_approver):
    try :
        i = 1
        while i <= total_app :
            app_name = commons.GetTextWithI(pr_rq.rq_vc["ap_name"],str(i))
            app_name = commons.ReplaceSpace(app_name)
...
def CheckReason(info_before,info_after,result,type_request):
...
def ResultNumber(number_before,number_after,msg):
...
def ResultBefore(number_before,number_after,msg):
            ...
            for j in range(2,8):
                ...
                if str(date_at_calendar.text) == str(request_date.day):
                    ...
                    if selected_date.rfind("[") > 0:
                        ...
                        #selected_date = commons.ReplaceSpace(selected_date[12: None])
...
def get_days_and_hour(data_column):
    ...
    if  commons.ReplaceSpace(data_column)  == "-":
...
def click_approver():
    result              = xpath.ParResultApprover()
        ...
        if total_app == 0:
            ...
            for i in range(1 , total_app + 1):
                ...
                if app_name.strip() == user_name.strip():
                    commons.CasePass(msg0[Pass][Des])
...
def ApproverIsApprovalException(type_request,info_before,result):
...
def ApproverIsSelect(total_approver,approver,result,info_after,content_approver,msg):
    ...
    while j <= total_approver :
        try :
            ...
            approver_name = commons.ReplaceSpace(approver_name)
            if approver_name == approver["approver_name"]:
                content_approver       = True
                result["rs_approver"]  = True
                info_after["approver"] = approver["approver_name"]
                commons.CasePass(msg[Pass][Des])
...
def check_approver_reason(type_request,approver,info_after,info_before):
    ...
    if type_request == All:
        ...
        # Check resaon #
        CheckReason(info_before,info_after,result,type_request)

        # Check approver #
        if approver["approval_exception"] == True:
            ApproverIsApprovalException(type_request,info_before,result)
        ...
        if total_cc == 0:
            ...
            while  i <= total_cc :
                ...
                if cc_name.strip() == selected_cc.strip():
                    result_cc = True
                    commons.CasePass(msgo[Pass][Des])
                    break
                i = i+1
            if result_cc == False :
                commons.WriteOnExcel(msgo[Fail])
            InfoCc(cc_name , selected_cc)
    else:
        commons.WriteOnExcel(msg[Fail])
...
def select_date_to_request_leave(Date_Used):
    commons.Title("Select Date") 
    result_click = False
...
def check_number_of_days_off(before,after,hour_use,vc_name,oneday,use_hour_unit,type_request):
    ...
    for vacation in before:
        vc_name = commons.ReplaceCustom(vc_name,"[Carry-over]")
        if vacation["vacation_name"] == vc_name :
            ...
            if vc_bf_use["total"] != "-" :
                ...
                Result_Before      = xpath.ResultBefore(vc_bf_use,days,msg_total)
                ResultBefore(**Result_Before)
            else :
                ResultBefore(**ResultBeforeOther)
            break
    ...
    for vacation in after: 
        if vacation["vacation_name"] == vc_name :
            ...
            if  vc_bf_use["total"] != "-" : 
                total  = ResultNumber(vc_af_use["total"],vc_bf_use["total"],msg_total)
            
                used_before_plus_used = change_hour_to_day(**UHourToDaysNone)
                used   = ResultNumber(vc_af_use["used"],used_before_plus_used,msg_used)

                remain_before_minus_used = change_hour_to_day(**RHourToDaysNone)
                remain = ResultNumber(vc_af_use["remain"],remain_before_minus_used,msg_remain)
                
                result_after(total,used,remain,msg_all)

            # vacation type is Other #
            else :
                total    = ResultNumber(vc_af_use["total"],"-",msg_total)
                used_before_plus_used =  change_hour_to_day(**UHourToDaysNone)
                used     = ResultNumber(vc_af_use["used"],used_before_plus_used,msg_used)
                remain   = ResultNumber(vc_af_use["remain"],"-",msg_remain)
                result_after(total,used,remain,msg_all)
        ...
        if commons.IsDisplayedByXpath(pr_rq.rq_vc["re_ic"] % str(i-1)) == True :
            ...
            if  Time_Cancel == True :
                ...
                if  commons.IsDisplayedByXpath(pr_rq.rq_vc["bt_cancel_request"]) == True :  
                    commons.CasePass(pr_rq.ClickOnCancelIcon[Pass][Des])
...
def CheckStatusAfterCancel(Total_Request,Request_To_Cancel,Status_Before_Cancel):
    ...
    while i<= Total_Request:
        Infor_Request         = info_request_list(i)
        Find_request_canceled = xpath.two_requests_are_the_same(Request_To_Cancel,Infor_Request)
...
def check_number_of_days_cancel(before,after,hour_use,vc_name,oneday,use_hour_unit,type_request):
    ...
    for vacation in before:
        if vacation["vacation_name"] == vc_name :
            ...
            if vc_bf_use["total"] != "-" :
                days = change_hour_to_day(**HourToDaysUsed)
                ResultBefore(vc_bf_use["total"],days,msg_cancel)
            else :
                ResultBefore(vc_bf_use["total"],"-",msg_cancel)
            break
...
def select_approver():
    ...
    if bt_select_approver == True:
        ...
        if commons.IsDisplayedByXpath(pr_rq.rq_vc["bt_quick_approver"]) == True: 
            ...
            list_app      = driver.find_elements_by_xpath(pr_rq.rq_vc["list_approver1"])
            total_app     = commons.TotalData(list_app)
            select_approver["approver_name"] = ApproverName(total_app,list_approver)
            return select_approver 
...
def select_cc_enter_reason():
    ...
    if commons.IsDisplayedByXpath( pr_rq.rq_vc["reason"]) == True:
        ...
        if reason.get_attribute('value') ==  pr_rq.rq_vc["reason_text"]:
            commons.CasePass(pr_rq.EnterReason[Pass][Des])
        else:
...
def check_use_hour_unit_and_half_day(total_vc):
...
def CheckCreatedRequest(Info_Vc,Type,Use_Hour_Unit,Approver):
    i      = 1
    result = False
    time.sleep(1)
    try:
        ...
        List_Request   = driver.find_elements_by_xpath(pr_rq.rq_vc["list_request"])
        Total_Request  = commons.TotalData(List_Request)
        MsgDisplayed   = type_vc.MsgDisplayedRequest(Type)
        if Total_Request >= 1 :
            while i <= Total_Request:
                Vc_Re = xpath.ParVacationRequest()
                if  Approver["approval_exception"] == True :
                    Vc_Re["status"]   = "Approved"
                    Info_Vc["status"] = "Approved"

                Vc_Re = VacationRequest(Vc_Re,i)
                ResultCondition = xpath.ResultCondition(Info_Vc,Vc_Re)
                if  ResultCondition == True :
                    result = True
                    commons.WriteOnExcel(MsgDisplayed[Pass])
                    ViewDetailRequest(Vc_Re,Type,Use_Hour_Unit,Approver,i)
                    break
                i = i+1
            if result == False:
...
def ViewDetailRequest(vc_rq,type_request,use_hour_unit,approver,i):
    ...
    commons.ClickElementWithXpath(data["rq_vc"]["ic_detail"] % (str(i-1) , str(i-1)))
    time.sleep(2)
...
def info_request_list(i):
    ...
    infor_request["vacation_name"] = xpath.LiRequest(i,"na")
...
def VacationDisplayedInTimeCard(Date_Request,Approver):
    try :
        i = 1
...
def ClickTypeRequest(Type):
    if  Type == Am :
        driver.find_element_by_xpath( pr_rq.rq_vc["radi_am"]).click()
    elif Type == Pm :
        driver.find_element_by_xpath( pr_rq.rq_vc["radi_pm"]).click()
    else :
        select_hour_use_hour_unit()

def choose_date_to_request(SelectVacation,Type,Date_Used):
    Vacation_Request = SelectVacation["vacation_request"]
    Result_Date = True
    ToDay       = str(datetime.date.today())
    if  Type == All:
        VcDate = select_date_to_request_leave(Date_Used)
        ...
        VcDate = select_date_to_request_leave(Date_Used)
        Result_Date , Vacation_Request = DateToRequest(VcDate,Vacation_Request,Result_Date,Type)
        if SelectVacation["result_select_vc"] != False :
            ClickTypeRequest(Type)
        ...
        Vacation_Request["request_date"] = ToDay
        VcDate =  select_date_to_request_leave(Date_Used)
        Vacation_Request["vc_date"] = DateToStr(VcDate) + " " + Vacation_Request["vc_date"]
        if VcDate == False:
            Result_Date = False
        if SelectVacation["result_select_vc"] != False :
            ClickTypeRequest(Type)
...
def TypeOfRequest(Type):
    if  Type == All :
        ...
        commons.Title("Request Vacation : Half Am ")
    elif Type == Pm :
        commons.Title("Request Vacation : Half Pm ")
    else :
        commons.Title("Request Vacation : Hour Unit ")
    
def AllChooseVacationToRequest(Vacation_Request,Type):
    commons.Title("Vacation Name")
    Result_Select_Vc = False
    Use_Hour_Unit    = False
    Total_Vc         = total_vacation()
    Infor_Vacation   = check_use_hour_unit_and_half_day(Total_Vc)
    ...
    while i <= Total_Vc:
        ...
        Vc_Use       = vacation_use_for_request()
        Hour_Use     = hour_used(Use_Hour_Unit,Type)
        Remain_Days  = Vc_Use["number_of_days"]
        Remain_Hours = Vc_Use["number_of_hours"]
        Vc_Name      = Vc_Use["vacation_name"]
        ...
        if float(Remain_Days) == 0 and float(Remain_Hours) == 0:
            ...
            Vacation_Request["vc_name"] = commons.ReplaceSpace(Vc_Name)
    ...
    ParChooseVacation = xpath.ParChooseVacation(Hour_Use,Result_Select_Vc,Use_Hour_Unit,Vacation_Request,Vc_Name)
    return ParChooseVacation

def HalfChooseVacationToRequest(vacation_request,Type):
    commons.Title("Vacation Name")
    Use_Half_Day     = 0
    Vc_Use_Half      = []
    Use_Hour_Unit    = False
    ...
    # Check there are any vacations using hour unit / half day 
    ...
    for i in range(1,len(Infor_Vacation)):
        if Infor_Vacation[i]["use_half_day"] == True:
            Vc_Use_Half.append(Infor_Vacation[i])
            Use_Half_Day = Use_Half_Day + 1

    if  Use_Half_Day == 0:
        if Type == Am :
            commons.WriteOnExcel(pr_rq.NoUseAm[Pass])
        else :
            commons.WriteOnExcel(pr_rq.NoUsePm[Pass])
    else:
        if Type == Am :
            Hour_use = hour_used(Use_Hour_Unit,Am)
        else :
            Hour_use = hour_used(Use_Hour_Unit,Pm)

        Result_Select_Vc = select_vacation_use_hour_unit_half_day(Total_Vc,Vc_Use_Half,Hour_use,Am)
        if  Result_Select_Vc != False :
            vacation_request["vc_name"] = commons.ReplaceSpace(Result_Select_Vc)
            commons.CasePass(pr_rq.SelectVacationName)
            commons.CasePass(data["vc_na"] % Result_Select_Vc )
            '''
            if Type == Am :
            ...
            if Type == Am :
                commons.WriteOnExcel(pr_rq.NotEnoughDayAm[Pass])
            else :
                commons.WriteOnExcel(pr_rq.NotEnoughDayPm[Pass])
    

    ParChooseVacation = xpath.ParChooseVacation(Hour_use,Result_Select_Vc,Use_Hour_Unit,vacation_request,Result_Select_Vc)
    return ParChooseVacation

def HourChooseVacationToRequest(vacation_request,Type):
    commons.Title("Vacation Name")
    Hour_Use         = 2
    Result_Unit      = 0
    Use_Hour_Unit    = False
    ...
    for i in range(1 , len(Infor_Vacation)):
        if Infor_Vacation[i]["use_hour_unit"] == True:
            Vc_Use_Hour_Unit.append(Infor_Vacation[i])
            Result_Unit = Result_Unit + 1

    if  Result_Unit == 0:
        commons.WriteOnExcel(pr_rq.NoUseHourUnit[Pass])
    else:
        Use_Hour_Unit    = True
        Result_Select_Vc = select_vacation_use_hour_unit_half_day(Total_Vc,Vc_Use_Hour_Unit,Hour_Use,Hour)
        if  Result_Select_Vc != False :
            commons.CasePass(pr_rq.SelectVacationName)
            commons.CasePass(data["vc_na"] % Result_Select_Vc )
            vacation_request["vc_name"] =  commons.ReplaceSpace(Result_Select_Vc)
...
def SelectVacationToRequest(VacationRequest,Type):
    if  Type == All or Type == Con :
        SelectVacation = AllChooseVacationToRequest(VacationRequest,Type)
    elif Type == Hour :
        SelectVacation = HourChooseVacationToRequest(VacationRequest,Type)
    else :
        SelectVacation = HalfChooseVacationToRequest(VacationRequest,Type)
    
    return SelectVacation

def Search(Info_User):
    time.sleep(3)
    # Use the currently logged in account to check the search function
    commons.ClickElementWithText("Request Vacation")
    driver.find_element_by_xpath( pr_rq.rq_vc["bt_select_cc"]).click()
    if  commons.IsDisplayedByXpath(pr_rq.rq_vc["bt_add_cc"]) == True :
        driver.find_element_by_xpath( pr_rq.rq_vc["dele_all_cc"]).click()
        List_Department  = driver.find_elements_by_xpath( pr_rq.rq_vc["org_search"])
        Total_Department = commons.TotalData(List_Department)

        # At the org  not showing data => Error displaying data of org
        if  Total_Department == 0 :
            commons.WriteOnExcel(pr_rq.NoDisplayData[Fail])
        else :
            Result_Input = commons.SendKey(data["user"],pr_rq.rq_vc["search"])
            if Result_Input == True :
                commons.CasePass(pr_rq.InputUserToSearch[Pass][Des])
                List_Department  = driver.find_elements_by_xpath( pr_rq.rq_vc["org_search"])
                Total_Department = commons.TotalData(List_Department)

                # After input the user's id to search => If results are not displayed
                if  Total_Department == 0 :
                    commons.WriteOnExcel(pr_rq.SearchUser[Fail])
                else :
                    # If the displayed results need to be checked, there are users searching
                    i       = 1 
                    Result  = False
                    while i <= Total_Department :
                        Name_Position = Info_User["Name"] + Info_User["Position"]
                        Name_Position = commons.ReplaceSpace(Name_Position)
                        Name_Position = commons.ReplaceCustom(Name_Position,"(")
                        Name_Position = commons.ReplaceCustom(Name_Position,")")
                        User          = commons.GetText(pr_rq.rq_vc["name_posi"] % str(i))
                        User          = xpath.User(User)
                        if Name_Position == User :
                            Result = True
                            break
                        i = i + 1
                    if  Result == True :
                        commons.WriteOnExcel(pr_rq.SearchUser[Pass])
                    else :
                        commons.WriteOnExcel(pr_rq.SearchUser[Fail])
            else:
                commons.WriteOnExcel(pr_rq.InputUserToSearch[Fail])

             
def Request(Approver,ResultApprover,Type,Msg,Condition):
    TypeOfRequest(Type)
    ...
    SelectVacation       = SelectVacationToRequest(VacationRequest,Type)
    ChooseDate           = choose_date_to_request(SelectVacation,Type,Date_Used)
    ...
    ParNumberOfDays      = xpath.ParNumberOfDays(NumberBeforeRequest,Type,SelectVacation)
    ...
    ConditionRequest     = xpath.ConditionRequest(SelectVacation,ResultApprover,ChooseDate)
    if  ConditionRequest == True :

        click_on_button_to_request()
        ResultRequest     = check_result_request()
        if  ResultRequest == Pass :
            information_vacation(VacationRequest)
            commons.WriteOnExcel(Msg[Pass])
            ...
            if requested == True :
                ...
                ParCancelRequest   = xpath.ParCancelRequest(SelectVacation)
                check_number_of_days_off(**ParNumberOfDays)
                if Type == All : 
                    ParTimeCard = xpath.ParTimeCard(Approver,ChooseDate)
                    VacationDisplayedInTimeCard(**ParTimeCard)
        ...
        commons.WriteOnExcel(Condition[Pass])
...
def RequestVacation():
    ...
    if TotalVc == 0 :
        ...
        ParamRequestAll  = xpath.ParamRequestAll(Approver,TypeApprover)
        ParamConsecutive = xpath.ParamRequestConsecutive(Approver,TypeApprover)
        ParamRequestAm   = xpath.ParamRequestAm(Approver,TypeApprover)
        ParamRequestPm   = xpath.ParamRequestPm(Approver,TypeApprover)
        ParamRequestHour = xpath.ParamRequestHour(Approver,TypeApprover)
        Request(**ParamRequestAll)
        Request(**ParamConsecutive)
        Request(**ParamRequestAm)
        Request(**ParamRequestPm)
        Request(**ParamRequestHour)


def MyVacationRequest():
    commons.Title("MY VACATION STATUS")
    ...
    if commons.IsDisplayedByXpath(pr_rq.rq_vc["vc_history"]) == True : 
        ...
        commons.WriteOnExcel(pr_rq.AccessSubmenuMy[Fail])
...
def MyVacation(Info_User):
    # try :
    #     RequestVacation()
    # except:
    #     commons.Content("Error xpath")
    # try :  
    #     Search(Info_User)
    # except:
    #     commons.Content("Error xpath")
